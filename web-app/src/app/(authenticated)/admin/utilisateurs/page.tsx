...
const INPUT = "w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-primary-light focus:outline-none focus:ring-2 focus:ring-primary-light/20";
...
export default function AdminUtilisateursPage() {
  ...
  function openEdit(u: CompteUtilisateur) {
    setForm({
      ...
      numero_praticien: u.numero_praticien || "",
      actif: u.actif,
    });
    setFormError("");
    setEditTarget(u);
  ...
  async function handleSave() {
    ...
    if (!form.nom || !form.prenom || !form.email || !form.role) {
      setFormError("Nom, prénom, courriel et rôle sont obligatoires");
      return;
    }
    if (modal === "create" && !form.password) {
      setFormError("Le mot de passe est obligatoire lors de la création");
      return;
    }
    setSaving(true);
    try {
      const body: Record<string, unknown> = {
        ...
        numero_praticien: form.numero_praticien || null,
      };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3">
        <input
          ...
          className="min-w-64 flex-1 rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-light/20"
        />
        <select
          value={filterRole}
        ...
        >
          ...
          <option value="false">Inactifs / En attente</option>
        </select>
      </div>
          ...
          <div className="w-full max-w-lg rounded-xl bg-white shadow-xl">
            ...
            <div className="max-h-[70vh] overflow-y-auto px-6 py-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="mb-1 block text-sm font-medium text-slate-700">Prénom <span className="text-red-500">*</span></label>
                  <input name="prenom" value={form.prenom} onChange={handleFormChange} required className={INPUT} />
                </div>
                <div>
                  <label className="mb-1 block text-sm font-medium text-slate-700">Nom <span className="text-red-500">*</span></label>
                  <input name="nom" value={form.nom} onChange={handleFormChange} required className={INPUT} />
                </div>
              </div>

              <div>
                <label className="mb-1 block text-sm font-medium text-slate-700">Courriel <span className="text-red-500">*</span></label>
              ...
              <div>
                <label className="mb-1 block text-sm font-medium text-slate-700">Rôle <span className="text-red-500">*</span></label>
